...
void UpdateSpeedValue(char *komenda,char *sliderValueChar);
void StringToInt(uint8_t *tab);//,int *encoderValueTab);// nie dziaLa
...
void StartDefaultTask(void const * argument);//Ekran
void StartUartTask(void const * argument);//Peryferia
/* USER CODE BEGIN PFP */
...
extern xQueueHandle QueueSend;
extern xQueueHandle QueueReceive;
extern xQueueHandle QueueSendSliderValue;
extern xQueueHandle QueueReceiveEncoderValue;
int sliderCurrenValue = 0;
//int encoderValue[]={0,0,0,0};
char sliderCurrentValueChar[4];
uint8_t sterowanieSend=0;//stm32->komputer
...
void StringToInt(uint8_t *tab)//,int *encoderValueTab)
{
int i = 0;
int j = 0;
int d = 0;
